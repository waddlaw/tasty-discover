...
spec_modules =
  describe "Test discovery"
    ...
    $ do
      let expectedTests =
            [ mkTest "PropTest.hs" "prop_additionAssociative",
...
spec_ignores =
  describe "Module ignore configuration"
    $ it "Ignores tests in modules with the specified suffix"
    $ do
...
spec_badModuleGlob =
  describe "Module suffix configuration"
    $ it "Filters discovered tests by specified suffix"
    $ do
...
spec_customModuleName =
  describe "Module name configuration"
    $ it "Creates a generated main function with the specified name"
    $ do
...
prop_mkModuleTree mtree =
  ...
  where
    ...
    flattenModule (mdl, (subTree, testVars)) =
      concat
